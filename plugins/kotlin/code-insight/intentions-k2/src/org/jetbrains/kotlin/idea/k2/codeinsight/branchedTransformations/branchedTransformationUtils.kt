 ...
 *  n is Int ..
 */
private fun KtWhenCondition.generateNewConditionWithSubject(subject: KtExpression?): KtExpression {
 ...
 *  n == 0 || n == 3 ..
 */
internal fun KtPsiFactory.combineWhenConditions(conditions: Array<KtWhenCondition>, subject: KtExpression?) = when (conditions.size) {
    0 -> null
    1 -> conditions[0].generateNewConditionWithSubject(subject)
    else -> buildExpression {
        appendExpressions(conditions.map { it.generateNewConditionWithSubject(subject) }, separator = "||")
    }
}